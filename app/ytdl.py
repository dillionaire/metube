...
class DownloadInfo:
    def __init__(
        ...
        self.title = (
            title if len(custom_name_prefix) == 0 else f"{custom_name_prefix}.{title}"
        )
        self.url = url
...
class Download:
    ...
    def __init__(
        ...
        if "impersonate" in self.ytdl_opts:
            self.ytdl_opts["impersonate"] = (
                yt_dlp.networking.impersonate.ImpersonateTarget.from_str(
                    self.ytdl_opts["impersonate"]
                )
            )
        self.info = info
        ...
        try:
            ...
            def put_status_postprocessor(d):
                ...
                # Capture all chapter files when SplitChapters finishes
                    ...
                    d.get("postprocessor") == "SplitChapters"
                    ...
                    if chapters:
                        for chapter in chapters:
                            if isinstance(chapter, dict) and "filepath" in chapter:
                                log.info(
                                    f"Captured chapter file: {chapter['filepath']}"
                                )
                                self.status_queue.put(
                                    {"chapter_file": chapter["filepath"]}
                                )
                    else:
                        log.warning(
                            "SplitChapters finished but no chapter files found in info_dict"
                        )
    ...
    async def update_status(self):
        while True:
            ...
            if self.canceled:
                log.info(
                    f"Download {self.info.title} is canceled; stopping status updates."
                )
                return
            ...
            if "chapter_file" in status:
                ...
                existing = next(
                    (
                        cf
                        for cf in self.info.chapter_files
                        if cf["filename"] == rel_path
                    ),
                    None,
                )
                if not existing:
                    self.info.chapter_files.append(
                        {"filename": rel_path, "size": file_size}
                    )
...
class PersistentQueue:
    ...
    def repair(self):
        # check DB format
        type_check = subprocess.run(["file", self.path], capture_output=True, text=True)
        ...
        try:
            ...
            log.debug(
                f"PersistentQueue:{self.identifier} backup failed, skipping repair"
            )
            return
        ...
        if "gnu dbm" in db_type:
            ...
            try:
                with dbm.open(self.path, "w") as db:
                    for key in list(db.keys()):
                        if len(key) > 0 and all(b == 0x00 for b in key):
                            log.debug(
                                f"{log_prefix} deleting key of length {len(key)} (all NUL bytes)"
                            )
                            del db[key]
                            deleted += 1
                ...
                log.debug(
                    f"{log_prefix} failed: db type is dbm.gnu, but the module is not available (dbm.error; module support may be missing or the file may be corrupted)"
                )
            ...
            try:
                ...
                if result.stderr:
                    ...
                    log.debug(
                        f"{log_prefix}{result.stdout or ' was successful, no output'}"
                    )
            except FileNotFoundError:
...
class DownloadQueue:
    def __init__(self, config, notifier, state_dir=None):
        self.config = config
        self.notifier = notifier
        sd = state_dir or self.config.STATE_DIR
        self.queue = PersistentQueue("queue", sd + "/queue")
        self.done = PersistentQueue("completed", sd + "/completed")
        self.pending = PersistentQueue("pending", sd + "/pending")
    ...
    async def __start_download(self, download):
        ...
        async with self.semaphore:
            if download.canceled:
                log.info(
                    f"Download {download.info.title} was canceled, skipping start."
                )
                return
                ...
                **(
                    {
                        "impersonate": yt_dlp.networking.impersonate.ImpersonateTarget.from_str(
                            self.config.YTDL_OPTIONS["impersonate"]
                        )
                    }
    ...
    def __calc_download_path(self, quality, format, folder):
        ...
        if folder:
            if not self.config.CUSTOM_DIRS:
                return None, {
                    "status": "error",
                    "msg": "A folder for the download was specified but CUSTOM_DIRS is not true in the configuration.",
                }
    ...
    async def __add_download(self, dl, auto_start):
        dldirectory, error_message = self.__calc_download_path(
            dl.quality, dl.format, dl.folder
        )
        if error_message is not None:
            return error_message
        output = (
            self.config.OUTPUT_TEMPLATE
            if len(dl.custom_name_prefix) == 0
            else f"{dl.custom_name_prefix}.{self.config.OUTPUT_TEMPLATE}"
        )
        ...
        if playlist_item_limit > 0:
            log.info(
                f"playlist limit is set. Processing only first {playlist_item_limit} entries"
            )
    ...
    async def __add_entry(
        ...
        error = None
        if (
            "live_status" in entry
            and "release_timestamp" in entry
            and entry.get("live_status") == "is_upcoming"
        ):
            dt_ts = datetime.fromtimestamp(entry.get("release_timestamp")).strftime(
                "%Y-%m-%d %H:%M:%S %z"
            )
        ...
        if etype.startswith("url"):
            ...
            if playlist_item_limit > 0:
                log.info(
                    f"Item limit is set. Processing only first {playlist_item_limit} entries"
                )
                entries = entries[:playlist_item_limit]
            for index, etr in enumerate(entries, start=1):
                etr["_type"] = "video"
                etr[etype] = (
                    entry.get("id") or entry.get("channel_id") or entry.get("channel")
                )
                etr[f"{etype}_index"] = "{{0:0{0:d}d}}".format(index_digits).format(
                    index
                )
            ...
            etype.startswith("url") and "id" in entry and "title" in entry
        ):
    ...
    async def add(
        ...
        log.info(
            f"adding {url}: {quality=} {format=} {already=} {folder=} {custom_name_prefix=} {playlist_item_limit=} {auto_start=} {split_by_chapters=} {chapter_template=}"
        )
